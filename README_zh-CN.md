...
UAL 旨在成为通用标准，我们非常欢迎社区贡献新的领域词库！

### 如何提交新词库？

为了保证语义 ID 的全局唯一性，我们提供了一套包管理工具来规范扩展流程。

#### 1. 创建扩展包
使用 `pkg_manager.py` 创建你的领域包（例如金融领域）：

```bash
# 在 src/ual/packages/ 生成 finance_v1.yaml
python3 tools/pkg_manager.py create finance_v1 --start-id 0x6000
```
...
concepts:
  0x6001: ["invoice", "bill"]
  0x6002: ["payment", "transaction"]
```

#### 3. 验证冲突
在提交前，务必运行验证工具检查 ID 是否冲突：

```bash
python3 tools/pkg_manager.py validate finance_v1.yaml
# ✅ Validation Passed: No ID conflicts detected.
```

#### 4. 提交回传
将你的 `.yaml` 文件提交 Pull Request 到本仓库。合并后，这些 ID 将被全球 UAL 网络识别！
